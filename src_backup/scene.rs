...
impl Scene {
    pub fn new(shader: render_gl::Shader, level: &level::Level, ctx: &render_gl::context::Context) -> Result<Scene, failure::Error> {

        let mut border_positions = Vec::<na::Vector3::<f32>>::new();

        let clr = na::Vector3::new(0.6, 0.5, 0.4);
        //TODO should be model ids
        let border = cube::Cube::new(shader, clr, &ctx.gl)?;
