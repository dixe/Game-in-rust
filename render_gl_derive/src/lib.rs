...
fn generate_impl(ast: &syn::DeriveInput) -> quote::Tokens {
    ...
    let fields_vertex_attrib_pointer = generate_vertex_attrib_pointer(&ast.body);
...
fn generate_vertex_attrib_pointer(body: &syn::Body) -> Vec<quote::Tokens> {
    match body {
        &syn::Body::Enum(_)
...
fn generate_struct_field_vertex_attrib_pointer_call(field: &syn::Field) -> quote::Tokens {
    ...
    let location_value_literal = match location_attr.value {
        ...
        _ => panic!("Field {} location attribute value must be and integer", field_name),
    };
