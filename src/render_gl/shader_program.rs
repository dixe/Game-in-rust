...
impl Program {
    pub fn from_res(gl: &gl::Gl, res: &Resources, name: &str) -> Result<Program, Error> {
        ...
        let shaders = POSSIBLE_EXT.iter()
            .map(|file_extension| {
                ShaderSource::from_res(gl, res, &format!("{}{}", name, file_extension))
            })
...
impl ShaderSource {
    ...
    pub fn from_res(gl: &gl::Gl, res: &Resources, name: &str) -> Result<ShaderSource, Error> {
        ...
        ShaderSource::from_source(gl, &source, shader_kind).map_err(|message| Error::CompileError {
            name: name.into(),
            message
        })
